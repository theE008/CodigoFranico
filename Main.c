...
int main (int argc, char *argv []) 
{
    if (argc < 2) 
    {
        // sobre o Codigo Franico / como usar
    }
    else
    {
        char *entrada = argv[1];
        ...
        //printf (entrada); // retorna todo o caminho separado por /

        int quantos = 0;

        char** partes = separar (entrada, '\\', &quantos);

        //imprimir_partes (entrada, '\\');

        //printf ("%s", partes [quantos - 1]); imprime o nome e a extensão correta do arquivo

        char* nome_do_arquivo = partes [quantos - 1];

        loop (quantos, x) limpar (partes [x]); limpar (partes);

        boolean eh_para_descriptografar = false;

        IF (eh_igual (nome_do_arquivo, "FRANICO.TXT")) eh_para_descriptografar = true;
